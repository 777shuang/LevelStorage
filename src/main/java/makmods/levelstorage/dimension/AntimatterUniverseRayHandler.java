...
public class AntimatterUniverseRayHandler implements ITickHandler {
    ...
    public void spawnDirtyTricks(World world) {
        ...
        if (rng.nextInt(10) == 0) {
            ...
            ptr.tZ = coordinateZ;
            PacketDispatcher
                .sendPacketToAllInDimension(PacketTypeHandler.populatePacket(ptr), world.provider.dimensionId);
            CommonHelper.spawnLightning(world, coordinateX, coordinateY, coordinateZ, false);
        }
    }

    @Override
    public void tickStart(EnumSet<TickType> type, Object... tickData) {
        if (!(tickData[0] instanceof World)) return;
            ...
            if (w.playerEntities.size() > 0) {
                ...
                for (Object o : w.playerEntities) {
                    ...
                    if (w.rand.nextInt(30) == 0) {
                        ...
                        w.createExplosion(null, e.posX, e.posY, e.posZ, 10.0F, true);
                        e.motionY *= 1.75F;
                    }
                }
            }

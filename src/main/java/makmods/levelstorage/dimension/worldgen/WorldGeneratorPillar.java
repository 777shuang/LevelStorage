...
public class WorldGeneratorPillar implements IWorldGenerator {
    ...
    static {
        Block[] defaults = { Block.oreCoal, Block.oreDiamond, Block.oreEmerald, Block.oreGold, Block.oreIron,
            Block.oreLapis, Block.oreRedstone };
        for (Block def : defaults) toGenerate.add(def);
        try {
                ...
                try {
                    if (s.startsWith("ore")) {
                        ...
                        for (ItemStack st : stacksForGiven) {
                            if (st == null) continue;
                            if (st.getItem() instanceof ItemBlock) toGenerate.add(Block.blocksList[st.itemID]);
                        }
                    }
                } catch (Exception e) {
    ...
    public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator,
        ...
        if (random.nextInt(RARITY) == 0) {
            ...
            if (genY > 0) {
                ...
                int THICKNESS = (int) Math.ceil((float) height / (float) 40 * (float) 3);
                for (int i = 0; i < height; i++) {
                    for (int x = -(THICKNESS / 2) + 1; x < THICKNESS / 2; x++) {
                        for (int z = -(THICKNESS / 2) + 1; z < THICKNESS / 2; z++) {
                            ...
                            int yT = genY + i;
                            int zT = genZ + z;
                            int idToGenerate = toGenerate.get(random.nextInt(toGenerate.size())).blockID;
                    ...
                    for (int x = -(THICKNESS / 2); x <= THICKNESS / 2; x++) {
                        for (int z = -(THICKNESS / 2); z <= THICKNESS / 2; z++) {
                            ...
                            int yT = genY + i;
                            int zT = genZ + z;
                            if (world.isAirBlock(xT, yT, zT)) world.setBlock(xT, yT, zT, wrapper.blockID);
                        }
                    }
                }
            }

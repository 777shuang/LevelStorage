...
public class BlockRockDesintegrator extends BlockMachineStandart implements IHasRecipe {
    ...
    public void addCraftingRecipe() {
        ...
        for (FluidContainerData d : FluidContainerRegistry.getRegisteredFluidContainerData()) {
            if (d.fluid.fluidID == FluidRegistry.WATER.getID()) waters.add(d.filledContainer.copy());
            if (d.fluid.fluidID == FluidRegistry.LAVA.getID()) lavas.add(d.filledContainer.copy());
        }
        for (ItemStack lava : lavas) for (ItemStack water : waters) {
            Recipes.advRecipes.addRecipe(
                new ItemStack(LSBlockItemList.blockRockDesintegrator),
                "lll",
                "cmc",
                "www",
                Character.valueOf('l'),

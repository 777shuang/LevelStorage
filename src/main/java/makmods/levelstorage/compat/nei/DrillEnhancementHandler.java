...
public class DrillEnhancementHandler extends TemplateRecipeHandler {
    ...
    public String getRecipeName() {
        return StatCollector.translateToLocal("other.drillEnhancementRecipeNEI");
    }
    ...
    public ItemStack createBookForEnhancement(EnhancementUtility ut) {
        if (ut == EnhancementUtility.SILKTOUCH) return createEnchantedBook(Enchantment.silkTouch, 1);
        else if (ut == EnhancementUtility.FORTUNE_1) return createEnchantedBook(Enchantment.fortune, 1);
        else if (ut == EnhancementUtility.FORTUNE_2) return createEnchantedBook(Enchantment.fortune, 2);
        else if (ut == EnhancementUtility.FORTUNE_3) return createEnchantedBook(Enchantment.fortune, 3);
        return null;
    }

    @Override
    public void loadCraftingRecipes(ItemStack result) {
        EnhancementUtility enhPresent = EnhancementUtility.readDrillEnhancement(result);
        if (enhPresent == null) {
            return;
        }
        ...
        if (enchantedBook == null) {
            System.out.println("[DrillEnhancementHandler] (NEI support): enchantedBook is null! This must be a bug!");
            return;
        } else {
            arecipes.add(
                new CachedShapedRecipe(
                    2,
                    2,
                    new Object[] { drill, null, null, enchantedBook },

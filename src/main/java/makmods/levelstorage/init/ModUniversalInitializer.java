...
public class ModUniversalInitializer {
    ...
    public FieldKind getKindOfField(Field f) {
        ...
        if (Item.class.isAssignableFrom(clazz)) return FieldKind.ITEM;
        else if (Block.class.isAssignableFrom(clazz)) return FieldKind.BLOCK;
    ...
    public boolean resolveRegisterWith(Field f) {
        ...
        if (el.isAnnotationPresent(RegisterWith.class)) {
            ...
            if (loadWith != null) return Loader.isModLoaded(loadWith);
        }
        return true;
    }
    ...
    public void create(Field f) {
        ...
        try {
            if (!resolveRegisterWith(f)) return;
            Class c = f.getType();
            int id = 0;
            FieldKind fk = getKindOfField(f);
            if (fk == FieldKind.BLOCK) id = LevelStorage.configuration.getBlock(f.getName(), getNextBlockID())
                .getInt();
            else if (fk == FieldKind.ITEM) id = LevelStorage.configuration.getItem(f.getName(), getNextItemID())
                .getInt();
            else LogHelper.severe("object is neither item nor block. This is a bug!");
            Constructor con = c.getConstructor(int.class);
            f.set(null, con.newInstance(id));
            Object obj = f.get(null);
            if (obj instanceof Block) ((Block) obj).setUnlocalizedName(f.getName());
            if (obj instanceof Item) ((Item) obj).setUnlocalizedName(f.getName());
    ...
    private Class<? extends ItemBlock> getPreferredItemBlock(Field f) {
        ...
        CustomItemBlock customIBAnn = (CustomItemBlock) blockClass.getAnnotation(CustomItemBlock.class);
        if (customIBAnn != null) {
            LogHelper.info(
                String.format(
                    "Block %s has @CustomItemBlock annotation (ItemBlock is %s), using it",
                    f.getName(),
                    customIBAnn.itemBlock()
                        .getCanonicalName()));
    ...
    private void registerBlock(Field f) {
        try {
            ...
            if (obj instanceof Block) {
                LogHelper.info("Registering block " + f.getName() + " in GameRegistry");
                Block currBlock = (Block) obj;
                String name = f.getName();
                GameRegistry.registerBlock(currBlock, getPreferredItemBlock(f), name);
            }
    ...
    private void addRecipe(Field f) {
        ...
        if (!Modifier.isPrivate(f.getModifiers())) {
            ...
            try {
                ...
                LogHelper.severe(
                    "Failed to add recipe for " + f.getType()
                        .getSimpleName());
                e.printStackTrace();
                return;
            }
            if (!(obj instanceof IHasRecipe)) {
                LogHelper.warning(
                    "Item/Block has no recipe: " + f.getType()
                        .getSimpleName());
                return;
            }
            Property p4 = LevelStorage.configuration.get(LevelStorage.RECIPES_CATEGORY, f.getName(), true);
            ...
            if (enable) {
                ...
                LogHelper.info(String.format("Recipe disabled for: %s (config)", f.getName()));
            }
    ...
    private void setBlockMiningLevels(Field f) {
        ...
        try {
            Object obj = f.get(null);
            if (obj instanceof Block) {
                LogHelper.info("Setting harvest level for block: " + f.getName());
    ...
    public void init() {
        ...
        for (Field f : items) this.create(f);
        for (Field f : items) this.registerBlock(f);
        for (Field f : items) this.addRecipe(f);
        for (Field f : items) this.setBlockMiningLevels(f);
    }
}

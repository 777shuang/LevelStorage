...
public class IVRegistry {
    ...
    public void recognizeOreDict() {
        try {
            ...
            for (String s : names) {
                try {
                    if (DEBUG) LogHelper.info("OreDict entry: " + s);
                    ArrayList<ItemStack> stacksForGiven = OreDictionary.getOres(s);
                    ...
                    LogHelper.severe(
                        "Exception trying to parse OreDict by IV registry. This is fatal error and unrecoverable.");
    ...
    public List<IVEntry> copyRegistry() {
        ...
        for (IVOreDictEntry entry : oreDictEntries) newList.add(entry);
        for (IVItemStackEntry entry : itemStackEntries) newList.add(entry);
        return newList;
    }
    ...
    public void printContents() {
        ...
        for (IVEntry entry : entries) {
            if (entry instanceof IVOreDictEntry) {
                ...
                LogHelper.info("\tOre Dictionary: " + ode.getName() + ", value: " + ode.getValue());
            } else if (entry instanceof IVItemStackEntry) {
                IVItemStackEntry ise = (IVItemStackEntry) entry;
                LogHelper.info(
                    "\tItemStack: " + ise.getStack()
                        .getDisplayName() + ", value: " + ise.getValue());
            }
        }
    }
    ...
    private void initCriticalNodes() {
        ...
        if (LevelStorage.configuration.get(LevelStorage.BALANCE_CATEGORY, "disableBlazeRodToPowderExploit", true)
            .getBoolean(true)) assign(new ItemStack(Item.blazePowder), 307);
        ...
        if (FluidRegistry.isFluidRegistered("oil")) assign(FluidRegistry.getFluid("oil"), 6);
        if (FluidRegistry.isFluidRegistered("bioethanol")) assign(FluidRegistry.getFluid("bioethanol"), 3);
        if (Loader.isModLoaded("gregtech_addon")) IVCrossMod.addGTValues();
        if (Loader.isModLoaded("AdvancedSolarPanel")) IVCrossMod.addASPValues();
    }

    public void assignFluid(Fluid f, int value) {
        String name = f.getUnlocalizedName();
        int valueActual = LevelStorage.configuration.get(IV_CATEGORY, name, value)
            .getInt();
        if (valueActual > 0) fluidEntries.add(new IVFluidEntry(f, valueActual));
    }

    public void assign(Object obj, int value) {
        if (obj instanceof ItemStack) assignItemStack((ItemStack) obj, value);
        else if (obj instanceof String) assignOreDictionary((String) obj, value);
        else if (obj instanceof Item) assignItemStack(new ItemStack((Item) obj), value);
        else if (obj instanceof Block) assignItemStack(new ItemStack((Block) obj), value);
        else if (obj instanceof Fluid) {
            assignFluid((Fluid) obj, value);
        } else throw new RuntimeException(
            "IVRegistry.assign() - obj's type is incorrect (" + obj.getClass()
                .getCanonicalName() + ")");
    }
    ...
    public void assignAll(Class<?> baseClass, int value) {
        for (Block b : Block.blocksList) {
            if (b != null) {
                if (baseClass.isAssignableFrom(b.getClass())) assignItemStack(new ItemStack(b), value);
            }
        }
    ...
    public void assignItemStack(ItemStack stack, int value) {
        ...
        int valueActual = LevelStorage.configuration
            .get(
                IV_CATEGORY,
                id + ":" + meta,
                value,
                stack.hasDisplayName() ? stack.getDisplayName()
                    .replace(".name", "") : stack.getUnlocalizedName())
            .getInt();
        if (valueActual > 0) itemStackEntries.add(new IVItemStackEntry(stack.copy(), value));
    }
    ...
    public void removeIV(Object obj) {
        ...
        if (obj instanceof String) {
            ...
            for (IVItemStackEntry entry : itemStackEntries) {
                ...
                if (entryStack.itemID == initialStack.itemID
                    && (entryStack.getItemDamage() == initialStack.getItemDamage()
                        || entryStack.getItemDamage() == OreDictionary.WILDCARD_VALUE)) {
                    toRemove = entry;
                    break;
                }
            }
        ...
        if (toRemove != null) {
            if (toRemove instanceof IVOreDictEntry) oreDictEntries.remove(toRemove);
            else itemStackEntries.remove(toRemove);
            IVRegistry.clearCache();
        } else LogHelper.severe(
            "removeIV: obj's type is incorrect - " + obj.getClass()
                .getSimpleName());
    }
    ...
    public void assignOreDictionary(String name, int value) {
        ...
        if (valueActual > 0) {
            ...
            for (ItemStack st : sts) assignItemStack(st.copy(), value);
            oreDictEntries.add(new IVOreDictEntry(name, value));
        }
    }
    ...
    public int getValueForItemStack(ItemStack st) {
        if (st == null) return NOT_FOUND;
        int baseValue = getValueFor(st);
        if (baseValue == NOT_FOUND) return NOT_FOUND;
    ...
    public static void clearCache() {
        if (DEBUG) {
            LogHelper.info(
                String
                    .format("Clearing IV cache, %d ItemStack, %d OreDict", itemStackCache.size(), oreDictCache.size()));
        }
    ...
    public int getValueFor_internal(Fluid fluid) {
        if (fluid == null) return NOT_FOUND;
        for (IVFluidEntry entry : fluidEntries) {
            if (entry.getFluid()
                .getName()
                .equals(fluid.getName())) return entry.getValue();
        }
        return NOT_FOUND;
    }
    ...
    public int getValueFor(Object obj) {
        if (!DO_CACHING) {
            if (obj instanceof String) return getValueFor_internal((String) obj);
            else if (obj instanceof ItemStack) return getValueFor_internal((ItemStack) obj);
            else if (obj instanceof Fluid) return getValueFor_internal((Fluid) obj);
        }
        if (obj instanceof String) {
            String odName = (String) obj;
            if (!oreDictCache.containsKey(odName)) oreDictCache.put(odName, getValueFor_internal(odName));
            ...
            if (!itemStackCache.containsKey(lst)) itemStackCache.put(lst, getValueFor_internal(objIS));
    ...
    public int getValueFor_internal(String name) {
        for (IVOreDictEntry oreDictEntry : oreDictEntries) {
            if (oreDictEntry.getName()
                .equals(name)) return oreDictEntry.getValue();
            if (!nameToItemStackMap.containsKey(name)) return NOT_FOUND;
            ...
            for (IVItemStackEntry ivItemStackEntry : itemStackEntries) {
                ...
                if (stack.itemID == toResolveIS.itemID && (stack.getItemDamage() == toResolveIS.getItemDamage()
                    || stack.getItemDamage() == OreDictionary.WILDCARD_VALUE)) return ivItemStackEntry.getValue();
            }
        }
        return NOT_FOUND;
    }
    ...
    public int getValueFor_internal(ItemStack objStack) {
        if (objStack == null) return NOT_FOUND;
        for (IVItemStackEntry itemStackEntry : itemStackEntries) {
            ItemStack iterationIS = itemStackEntry.getStack();
            if (objStack.itemID == iterationIS.itemID && (iterationIS.getItemDamage() == objStack.getItemDamage()
                || iterationIS.getItemDamage() == OreDictionary.WILDCARD_VALUE)) return itemStackEntry.getValue();
            String toResolveIS = null;
            for (Entry<ItemStack, String> entryIs : itemStackToNameMap.entrySet()) {
                ItemStack st = entryIs.getKey();
                if (st.itemID == objStack.itemID && (st.getItemDamage() == objStack.getItemDamage()
                    || st.getItemDamage() == OreDictionary.WILDCARD_VALUE)) {
            ...
            for (IVOreDictEntry oreDictEntry : oreDictEntries) {
                if (oreDictEntry.getName()
                    .equals(toResolveIS)) return oreDictEntry.getValue();

...
public class IVRecipeParser implements IRecipeParser {
    ...
    public static int PASSES = LevelStorage.configuration.get(
        IVRegistry.IV_CATEGORY,
    ...
    public int assignCrafting(ItemStack is) {
        ...
        if (recipesFor.size() == 0) return IVRegistry.NOT_FOUND;
        List<Integer> candidates = Lists.newArrayList();
        for (IRecipe recipe : recipesFor) {
            int fIV = 0;
            List<IWrappedRecipeCompound> iss = RecipeHelper.getKnownCompounds(recipe);
            ...
            for (IWrappedRecipeCompound rc : iss) {
                if (rc instanceof ItemStackRecipeCompound) {
                    ...
                    if (value != IVRegistry.NOT_FOUND) fIV += value;
            ...
            if (recipe.getRecipeOutput().stackSize > 0) fIV /= recipe.getRecipeOutput().stackSize;
            if (fIV > 0 && !hasUnknown) candidates.add(fIV);
        }
        int toReleased = Integer.MAX_VALUE;
        for (Integer candidate : candidates)
            if (candidate < toReleased && (candidate != IVRegistry.NOT_FOUND)) toReleased = candidate;
        if (toReleased == Integer.MAX_VALUE) {
            ...
            if (oreDict != null) {
                ...
                if (odIV == IVRegistry.NOT_FOUND) {
                    IVRegistry.instance.assignOreDict_dynamic(oreDict, toReleased);
    ...
    public void assignIC2Machine(IMachineRecipeManager manager) {
        ...
        for (Entry<IRecipeInput, RecipeOutput> entry : recipes.entrySet()) {
            ...
            int inputValue = input instanceof RecipeInputItemStack
                ? IVRegistry.getValue(((RecipeInputItemStack) input).input)
                : IVRegistry.getValue(((RecipeInputOreDict) input).input);
            if (inputValue == IVRegistry.NOT_FOUND) continue;
            int iv = inputValue / output.items.get(0).stackSize;
            int outputIV = IVRegistry.getValue(output.items.get(0));
            if (outputIV == IVRegistry.NOT_FOUND) IVRegistry.instance.assignItemStack_dynamic(output.items.get(0), iv);
            else if (iv < outputIV && iv > 0) {
                IVRegistry.instance.removeIV(output.items.get(0));
                IVRegistry.instance.assignItemStack_dynamic(output.items.get(0), iv);
            }
        }
    }
    ...
    public void assignFurnace(FurnaceRecipes recipes) {
        ...
        for (IVEntry entry : copied) {
            if (!(entry instanceof IVItemStackEntry)) continue;
            ...
            if (outputSmelting != null) {
                ...
                if (alreadyExistingValue == IVRegistry.NOT_FOUND)
                    IVRegistry.instance.assignItemStack_dynamic(outputSmelting.copy(), iv);
    ...
    public void parse() {
        ...
        if (iterator > 0) LogHelper.info("Average ms per recipe: " + totalMS / iterator);
    }

...
public class RecipeHelper {
    ...
    public static List<IRecipe> getRecipesFor(ItemStack is) {
        ...
        for (IRecipe r : allRecipes) {
            ...
            if (recOutput != null) {
                ...
                if (recOutput.itemID == is.itemID && (recOutput.getItemDamage() == is.getItemDamage()
                    || is.getItemDamage() == OreDictionary.WILDCARD_VALUE)) recs.add(r);
            }
        }
    ...
    public static String resolveOreDict(ItemStack stack) {
        ...
        for (Entry<ItemStack, String> entryIs : IVRegistry.itemStackToNameMap.entrySet()) {
            ItemStack st = entryIs.getKey();
            if (st.itemID == stack.itemID && (st.getItemDamage() == stack.getItemDamage()
    ...
    public static Object[] examineAdvRecipe(Object obj) {
        ...
        try {
            ...
            LogHelper.severe(
                "Failed to examine Advanced Recipe. Either you're using a too old or a too new version of IC2 (most probably the latter");
            e.printStackTrace();
        }
        return null;
    }
    ...
    public static List<IWrappedRecipeCompound> getKnownCompounds(IRecipe recipe) {
        ...
        if (recipe instanceof ShapedRecipes)
            for (ItemStack stack : ((ShapedRecipes) recipe).recipeItems) ingrs.add(new ItemStackRecipeCompound(stack));
        else if (recipe instanceof ShapelessRecipes) {
            List<ItemStack> items = ((ShapelessRecipes) recipe).recipeItems;
            for (ItemStack stack : items) ingrs.add(new ItemStackRecipeCompound(stack));
            ...
            for (Object obj : ((ShapelessOreRecipe) recipe).getInput()) {
                if (obj instanceof ItemStack) {
                    ...
                    if (list.size() > 0) {
                        ...
                        if (oreDictName != null) ingrs.add(new OreDictRecipeCompound(oreDictName));
                    }

                } else {
                    LogHelper.severe(
                        "RecipeHelper.java: getKnownCompounds(): ShapelessOreRecipe - input compound is unknown. ("
                            + obj.getClass()
                                .getCanonicalName()
                            + ")");
                }
            ...
            for (Object obj : ((ShapedOreRecipe) recipe).getInput()) {
                if (obj instanceof ItemStack) {
                    ...
                    try {
                        LogHelper.severe(
                            "RecipeHelper.java: getKnownCompounds(): ShapedOreRecipe - input compound is unknown. ("
                                + obj.getClass()
                                    .getCanonicalName()
                                + ")");
                    } catch (Exception e) {}
            ...
            || recipe.getClass()
                ...
                .equals("AdvShapelessRecipe")) {
                    ...
                    if (contents == null) {
                        LogHelper.severe(
                            "Something really bad and weird happened when tried to parse AdvRecipe. (contents = null)");
                        return null;
                    }
                    for (Object obj : contents) {
                        if (obj != null) {
                            if (obj instanceof String) {
                                ...
                                LogHelper.severe(
                                    "RecipeHelper: AdvRecipe parsing: incorrect obj type - " + obj.getClass()
                                        .getSimpleName());
                            }
                        }
                    }
            ...
            LogHelper.warning(
                "Unknown recipe type: " + recipe.getClass()
                    .getCanonicalName());

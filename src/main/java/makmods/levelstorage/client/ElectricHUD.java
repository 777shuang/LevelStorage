...
public class ElectricHUD implements ITickHandler {
    ...
    public void renderHUD() {
        ...
        if (player != null && Minecraft.getMinecraft().currentScreen == null
            ...
            if (armorData.capacity != 0) percentArmor = (int) ((armorData.charge * 100.0f) / armorData.capacity);
            else percentArmor = 0;

            if (percentArmor > 0 && percentArmor <= 10) color = EnumChatFormatting.RED;
            else if (percentArmor > 10 && percentArmor <= 20) color = EnumChatFormatting.DARK_RED;
            else if (percentArmor > 20 && percentArmor <= 40) color = EnumChatFormatting.GOLD;
            else if (percentArmor > 40 && percentArmor <= 60) color = EnumChatFormatting.YELLOW;
            else color = EnumChatFormatting.WHITE;

            // ACTUAL RENDERING
            if (percentArmor > 0) {
                FontRenderer fr = Minecraft.getMinecraft().fontRenderer;
                fr.drawStringWithShadow(
                    color + StatCollector.translateToLocal("other.energyHUD") + " " + percentArmor + "%",
                    2,
                    2,
                    0);
            }

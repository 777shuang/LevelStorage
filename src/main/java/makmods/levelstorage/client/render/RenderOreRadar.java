    ...
    public void radarRender(DrawBlockHighlightEvent event) {
        ...
        if (ItemArmorTeslaHelmet.playerGetArmor(event.player) != null) {
                        ...
                        for (int z = -RENDER_DISTANCE; z < RENDER_DISTANCE; z++) {
                            try {
                                ...
                                if (!w.isAirBlock(currX, currY, currZ)) {
                                    int blockId = w.getBlockId(currX, currY, currZ);
                                    int meta = w.getBlockMetadata(currX, currY, currZ);
                                    Block bl = Block.blocksList[blockId];
                                    if (OreDictHelper.getOreName(new ItemStack(bl))
                                        .startsWith("ore")) {
                                        oreLoc.add(new BlockLocation(w.provider.dimensionId, currX, currY, currZ));
                                    }
                                }
                            } catch (Exception e) {
            ...
            for (BlockLocation bl : oreLoc) {
                int blockId = event.player.worldObj.getBlockId(bl.getX(), bl.getY(), bl.getZ());

...
public class PacketTeslaRay extends PacketLS {
    ...
    public static void issue(double x, double y, double z, double tX, double tY, double tZ) {
        ...
        ptr.tY = tY;
        ptr.tZ = tZ;
        PacketDispatcher.sendPacketToAllPlayers(PacketTypeHandler.populatePacket(ptr));
    }
    ...
    public void execute(INetworkManager network, Player player) {
        if (LevelStorage.getSide()
            .isClient()) {
            EntityPlayer p = (EntityPlayer) player;
            EnergyRayFX pe = new EnergyRayFX(p.worldObj, initX, initY, initZ, tX, tY, tZ, 48, 141, 255, 40);

...
public class LogicSlot {
    ...
    public boolean isItemTheSame(ItemStack stack) {
        return stack == null ? false : stack.itemID == get().itemID && stack.getItemDamage() == get().getItemDamage();
    }
    ...
    public boolean canAdd(int amount) {
        if (get() == null && amount <= boundInventory.getInventoryStackLimit()) return true;
        ItemStack alreadyThere = get();
        if (boundInventory.getInventoryStackLimit() - alreadyThere.stackSize >= amount) return true;
        return false;
    }
    ...
    public void add(int amount) {
        if (get() == null) throw new NullPointerException("HelperLogicSlot: add(): get() returns null");
    ...
    public boolean add(ItemStack what, boolean simulate) {
        if (get() != null) {
            if (get().getItemDamage() != what.getItemDamage() || get().itemID != what.itemID) {
                return false;
            } else {
                int freeSpace = what.getItem()
                    .getItemStackLimit() - get().stackSize;
                if (freeSpace >= what.stackSize) {
                    if (!simulate) {
                        ItemStack newIS = get().copy();
                        newIS.stackSize += what.stackSize;
                        boundInventory.setInventorySlotContents(index, newIS.copy());
                        return true;
                    }
                    return true;
                } else return false;
            ...
            if (!simulate) boundInventory.setInventorySlotContents(index, what.copy());
            return true;
        }
    }

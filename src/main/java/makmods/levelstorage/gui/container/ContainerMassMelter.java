...
public class ContainerMassMelter extends Container {
    ...
    public ContainerMassMelter(InventoryPlayer inventoryPlayer, TileEntityMassMelter te) {
        this.tileEntity = te;
    ...
    public void addBurnSlots() {
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                ...
                this.addSlotToContainer(new SlotChecked(tileEntity, j + i * 3, 63 + j * 18, 9 + i * 18));
            }
        }
    }
    ...
    public void detectAndSendChanges() {
        ...
        for (int i = 0; i < this.crafters.size(); i++) {
            ...
            icrafting.sendProgressBarUpdate(this, 4, this.tileEntity.getStored());
            icrafting.sendProgressBarUpdate(
                this,
                5,
                this.tileEntity.getFluidTank()
                    .getFluidAmount());
            icrafting.sendProgressBarUpdate(this, 6, this.tileEntity.getProgress());
            icrafting.sendProgressBarUpdate(this, 7, tileEntity.getMaxProgress());
        }
    }
        ...
        switch (index) {
            ...
            case 5: {
                tileEntity.getFluidTank()
                    .setFluid(new FluidStack(LSFluids.instance.fluidIV, value));
                break;
            }
            case 6: {
    ...
    public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < tileEntity.getSizeInventory()) {
                if (!this.mergeItemStack(slotItemStack, tileEntity.getSizeInventory(), inventorySlots.size(), false)) {
                    return null;
                }
            } else {
                if (IVRegistry.hasValue(slotItemStack)) {
                    if (!this.mergeItemStack(slotItemStack, 0, tileEntity.getSizeInventory(), false)) {
                        return null;
                    }
                }
            }

...
public class ContainerWithPhantomSlots extends Container {
    ...
    protected void fillPhantomSlot(Slot slot, ItemStack stackHeld, int mouseButton, int modifier) {
        if (!slot.isItemValid(stackHeld)) return;
        if (!(slot instanceof PhantomSlot)) return;
        int stackSize;
        if (((PhantomSlot) slot).isUnstackable()) stackSize = 1;
        else stackSize = mouseButton == 0 ? stackHeld.stackSize : 1;

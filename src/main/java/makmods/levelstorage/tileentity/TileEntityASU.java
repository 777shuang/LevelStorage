...
public class TileEntityASU extends TileEntityInventory
    implements IWrenchable, IEnergySource, IEnergySink, IEnergyStorage, ITEHasGUI {
    ...
    public void charge(LogicSlot ls, boolean charge) {
        if (ls.get() == null) return;
        if (!(ls.get()
            .getItem() instanceof IElectricItem)) return;
        IElectricItem electricItem = (IElectricItem) ls.get()
            .getItem();
        if (charge) {
            this.stored -= ElectricItem.manager
                .charge(ls.get(), (int) Math.min(Integer.MAX_VALUE, stored), TIER, false, false);
        } else {
            if (!electricItem.canProvideEnergy(ls.get())) return;
            this.stored += ElectricItem.manager
                .discharge(ls.get(), (int) Math.min(Integer.MAX_VALUE, EU_STORAGE - stored), TIER, false, false);
        }
    }
    ...
    public double injectEnergyUnits(ForgeDirection directionFrom, double amount) {
        if (this.stored > EU_STORAGE) return amount;

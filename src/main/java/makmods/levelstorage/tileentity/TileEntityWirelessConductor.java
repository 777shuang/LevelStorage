...
public class TileEntityWirelessConductor extends TileEntity
    implements IWirelessConductor, IInventory, IEnergyTile, IEnergySink, IWrenchable, IEnergySource, IHasButtons {
    ...
    public static void getConfig() {
        Property p = LevelStorage.configuration.get(Configuration.CATEGORY_GENERAL, "conductorsSpawnLightnings", true);
        p.comment = "If set to false, wireless conductors will stop spawning lightnings";
        ENABLE_LIGHTNINGS = p.getBoolean(true);
        Property p2 = LevelStorage.configuration.get(Configuration.CATEGORY_GENERAL, "conducorsSpawnParticles", true);
    ...
    public boolean canReceive(int amount) {
        if (this.energyToSend + amount <= MAX_ENERGY_INTERNAL) return true;
        return false;
    }
    ...
    public void handleButtonClick(int id) {
        switch (id) {
            case 1: {
                ...
                this.type = this.type == ConductorType.SINK ? ConductorType.SOURCE : ConductorType.SINK;
                break;
            }
        }
    }
    ...
    public double demandedEnergyUnits() {
        if (this.type == ConductorType.SOURCE) {
            if (this.safePair != null) {
                if (this.safePair.canReceive(MAX_PACKET_SIZE)) return MAX_PACKET_SIZE;
            }
        }
        return 0;
    }
    ...
    public double injectEnergyUnits(ForgeDirection directionFrom, double amount) {
        if (this.type == ConductorType.SOURCE) {
            if (this.safePair != null) {
                BlockLocation thisTe = new BlockLocation(this.getDimId(), this.getX(), this.getY(), this.getZ());
                BlockLocation pairTe = new BlockLocation(
                    this.safePair.getDimId(),
                    this.safePair.getX(),
                    this.safePair.getY(),
                    this.safePair.getZ());
                int amtWithDisc = (int) amount
                    - BlockLocation.getEnergyDiscount((int) amount, thisTe.getDistance(pairTe));
    ...
    public int receiveEnergy(int amount, IWirelessConductor transmitter) {
        ...
        if (this.type == ConductorType.SOURCE) return amount;
        else {
            this.elapsedReceives++;
            if (this.elapsedReceives > 600 && amount > 128) {

                if (ENABLE_LIGHTNINGS) {
                    CommonHelper.spawnLightning(this.worldObj, this.xCoord, this.yCoord, this.zCoord, false);
                }
            ...
            // }
            if (energyToSend + amount > MAX_ENERGY_INTERNAL) return MAX_ENERGY_INTERNAL - energyToSend;
            energyToSend += amount;
                ...
                if (te != null) {
                    if (te instanceof IWirelessConductor) {
                        if (te != this) {
                            ...
                            if (WirelessConductorRegistry.instance.isAddedToRegistry((IWirelessConductor) te)) {
                                ConductorType pairType = conductor.getType();
                                if (this.getType() != pairType) return conductor;
                            }
                        }
                    }
                }
    ...
    public void updateEntity() {
        ...
        if (!WirelessConductorRegistry.instance.isAddedToRegistry(this)) {
            WirelessConductorRegistry.instance.addConductorToRegistry(this, this.type);
        }
        if (WirelessConductorRegistry.instance.getConductorType(this) != this.type) {
            WirelessConductorRegistry.instance.setConductorType(this, this.type);
        }

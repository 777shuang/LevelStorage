...
public class TileEntityParticleAccelerator extends TileEntityInventorySinkWithFluid
    ...
    public static final int OPERATIONS_PER_TICK = 200 / IVRegistry.IV_TO_FLUID_CONVERSION.getKey();
    ...
    public boolean canFill(ForgeDirection from, Fluid fluid) {
        return fluid.getName()
            .equals(LSFluids.instance.fluidIV.getName());
    }
    ...
    public ItemStack getPatternIdeal() {
        if (sampleSlot.get() == null) return null;
        ItemStack is = sampleSlot.get()
            .copy();
        is.stackSize = 1;
        return is;
    }
    ...
    public void updateEntity() {
        super.updateEntity();
        if (!LevelStorage.isSimulating()) return;
        if (!canUse(ENERGY_COST_PER_TICK)) return;
        switch (mode) {
            case ANTIMATTER_PRODUCTION_MODE:
                if (handleAntimatterTick()) use(ENERGY_COST_PER_TICK);
                break;
            case MATTER_RESHAPING_MODE:
                if (handleMatterTick()) use(ENERGY_COST_PER_TICK);
                break;
        }
    }

    private boolean handleMatterTick() {
        if (!IVRegistry.hasValue(getPatternIdeal())) this.phantomInventory.setInventorySlotContents(0, null);
        int oldProgress = progress;
        boolean shouldUseEnergy = false;
        if (getPatternIdeal() == null) return false;
        if (!outputSlot.add(getPatternIdeal(), true)) return false;
        int ivCost = IVRegistry.getValue(getPatternIdeal());
        if (ivCost == IVRegistry.NOT_FOUND) return false;
        this.maxProgress = ivCost;
        for (int i = 0; i < OPERATIONS_PER_TICK; i++) {
            if (this.internalIV <= ivCost + 1) {
                FluidStack drained = this.getFluidTank()
                    .drain(IVRegistry.IV_TO_FLUID_CONVERSION.getValue(), true);
                if (drained != null && drained.amount == IVRegistry.IV_TO_FLUID_CONVERSION.getValue()) {
                    this.internalIV += IVRegistry.IV_TO_FLUID_CONVERSION.getKey();
                }
            } else {
    ...
    private boolean handleAntimatterTick() {
        boolean shouldUseEnergy = false;
        int oldProgress = progress;
        if (!outputSlot.add(getAntimatterIdeal(), true)) return false;

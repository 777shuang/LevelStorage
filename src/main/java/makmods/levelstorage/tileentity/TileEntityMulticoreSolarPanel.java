...
public class TileEntityMulticoreSolarPanel extends TileEntity implements IEnergyTile, IWrenchable, IEnergySource,
    IInventory, INetworkClientTileEntityEventListener, INetworkDataProvider, INetworkUpdateListener {
    ...
    public void updateEntity() {
        ...
        if ((lastX != this.xCoord) || (lastZ != this.zCoord) || (lastY != this.yCoord)) {
            lastX = this.xCoord;
            lastY = this.yCoord;
            lastZ = this.zCoord;
        ...
        for (int i = 0; i < chargeSlots.length; i++) {
            if ((chargeSlots[i] != null) && (Item.itemsList[chargeSlots[i].itemID] instanceof IElectricItem)
                && (storage > 0)) {
                sentPacket = ElectricItem.manager.charge(chargeSlots[i], storage, solarType, false, false);
    ...
    public void updateVisibility() {
        Boolean rainWeather = wetBiome && (worldObj.isRaining() || worldObj.isThundering());
        ...
        if (!worldObj.canBlockSeeTheSky(xCoord, yCoord + 1, zCoord) || (noSunWorld)) {
            skyIsVisible = false;
        } else {
            skyIsVisible = true;
        }
    ...
    public void readFromNBT(NBTTagCompound nbttagcompound) {
        ...
        for (int i = 0; i < nbttaglist.tagCount(); i++) {
            ...
            if (j >= 0 && j < chargeSlots.length) {
                chargeSlots[j] = ItemStack.loadItemStackFromNBT(nbttagcompound1);
            }
        }
    }

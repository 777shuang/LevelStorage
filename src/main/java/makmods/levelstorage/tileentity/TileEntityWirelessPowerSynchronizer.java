...
public class TileEntityWirelessPowerSynchronizer extends TileEntity
    implements IHasTextBoxes, IHasButtons, IEnergyTile, IEnergySink, IWrenchable, IEnergySource, IEnergyStorage {
    ...
    public double getOfferedEnergy() {
        if (this.deviceType == SyncType.RECEIVER) return Math.min(MAX_PACKET_SIZE, internalBuffer);
        else return 0;
    }
    ...
    public void updateEntity() {
        ...
        if (this.deviceType.equals(SyncType.RECEIVER)) return;
    }
    ...
    public double injectEnergyUnits(ForgeDirection directionFrom, double amount) {
        if (this.deviceType.equals(SyncType.RECEIVER)) return amount;
        ...
        if (mutableRightSyncList.size() == 0) return amount;
        ...
        for (Object pSyncValid : mutableRightSyncList) {
            if (pSyncValid instanceof TileEntityWirelessPowerSynchronizer)
                ((TileEntityWirelessPowerSynchronizer) pSyncValid).addEnergy(forEach);
            notUsedUp -= forEach;
        }
        return notUsedUp;
    }
    ...
    public void readFromNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        this.deviceType = par1nbtTagCompound.getBoolean("isTransmitter") ? SyncType.TRANSMITTER : SyncType.RECEIVER;
    ...
    public void writeToNBT(NBTTagCompound par1nbtTagCompound) {
        ...
        par1nbtTagCompound.setBoolean("isTransmitter", this.deviceType == SyncType.TRANSMITTER);

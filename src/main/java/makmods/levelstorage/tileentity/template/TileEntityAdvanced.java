...
public abstract class TileEntityAdvanced extends TileEntity
    implements IEnergySource, IEnergyStorage, IEnergySink, IWrenchable, IInventory, IFluidHandler {
    ...
    public TileEntityAdvanced(int inventorySize, boolean energyNetTE, boolean acceptsEnergy, boolean emitsEnergy,
        int maxStorage, int eut, int tankVolume, boolean fluidTE) {
        super();
    ...
    private void unloadFromENet() {
        if (energyNetTE) if (LevelStorage.isSimulating()) if (addedToENet) {
    ...
    public FluidTankInfo[] getTankInfo(ForgeDirection from) {
        if (!fluidTE) return null;
        else return new FluidTankInfo[] { tank.getInfo() };
    }
    ...
    public double injectEnergyUnits(ForgeDirection directionFrom, double amount) {
        if (stored + amount > maxStorage) return amount;
    ...
    public double getOfferedEnergy() {
        if (!emitsEnergy) return 0;
        else return Math.min(stored, eut);
    }

...
public class TileEntityMassMelter extends TileEntityInventorySinkWithFluid implements ISidedInventory, ITEHasGUI {

    private int progress;
    public int ivBuffer = 0;
    public static final int EU_PER_IV = 30;
    public static final int OPERATIONS_PER_TICK = 100 / IVRegistry.IV_TO_FLUID_CONVERSION.getKey();
    ...
    public TileEntityMassMelter() {
        ...
        for (int i = 0; i < slots.length; i++) slots[i] = new LogicSlot(this, i);
    }
    ...
    public void updateEntity() {
        ...
        if (!canUse(EU_PER_IV)) return;
        for (int i = 0; i < OPERATIONS_PER_TICK; i++) {
            if (!canUse(EU_PER_IV)) break;
            if (this.ivBuffer < IVRegistry.IV_TO_FLUID_CONVERSION.getKey()) {
                int plusBuffer = 0;
                {
                    for (LogicSlot slot : slots) {
                        ...
                        {
                            if (slot.get() != null) {
                                ItemStack gauge = slot.get()
                                    .copy();
                                gauge.stackSize = 1;
                                int ivFor = IVRegistry.instance.getValueFor(gauge);
                                slot.consume(1);
                ...
                int freeSpaceInTank = this.getFluidTank()
                    .getCapacity()
                    - this.getFluidTank()
                        .getFluidAmount();
                if (freeSpaceInTank >= 0) {
                    if (this.fill(
                        ...
                        new FluidStack(LSFluids.instance.fluidIV, IVRegistry.IV_TO_FLUID_CONVERSION.getValue()),
                        true) != 0) {
                        use(EU_PER_IV);
                        this.ivBuffer -= IVRegistry.IV_TO_FLUID_CONVERSION.getKey();
                        freeSpaceInTank--;
                    }
                } else break;
            }
        }
        this.setProgress(ivBuffer >= IVRegistry.IV_TO_FLUID_CONVERSION.getValue() ? ivBuffer : 0);
    }
    ...
    public boolean canInsertItem(int slot, ItemStack itemstack, int side) {
        ...
        if (dir == ForgeDirection.UP) return isItemValidForSlot(slot, itemstack);
        return false;
    }

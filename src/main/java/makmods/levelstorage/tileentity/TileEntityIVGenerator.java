...
public class TileEntityIVGenerator extends TileEntityInventory implements IFluidHandler, ITEHasGUI {
    ...
    public static void getConfig() {
        IV_PER_TICK = Config
            .getInt(LSConfigCategory.IV, "IVGenerator_Rate", 20, "Speed at which IV Generators produce IV. (per tick)");
    }
    ...
    public void updateEntity() {
        ...
        ticker++;
        if (ticker % tickRate() == 0) updateVisibility();
        boolean shouldProduce = skyIsVisible && sunIsUp;
        if (shouldProduce) produceIV();
    }

    public int calculateBuffIV() {
        if (upgradeSlot.get() == null) return 0;
        if (!isItemValidForSlot(0, upgradeSlot.get())) return 0;
        ItemStack is = upgradeSlot.get();
        return BUFF_IV_T * is.stackSize;
    }

    public void produceIV() {
        if (this.tank.getFluidAmount() == this.tank.getCapacity()) return;
        ...
        this.internalIVTicker -= mb
            * this.tank.fill(new FluidStack(LSFluids.instance.fluidIV, (int) (internalIVTicker / (double) mb)), true);
    }

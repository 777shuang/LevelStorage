...
public class TileEntityLavaFabricator extends TileEntityInventorySinkWithFluid implements ITEHasGUI, ISidedInventory {
    ...
    public boolean isValidInputInSlot(ItemStack itemstack) {
        ...
        ItemStack filled = FluidContainerRegistry
            .fillFluidContainer(new FluidStack(FluidRegistry.LAVA, FluidContainerRegistry.BUCKET_VOLUME), copy);
        return filled != null;
    }

    @Override
    public boolean isItemValidForSlot(int i, ItemStack itemstack) {
        if (i == 0) return isValidInputInSlot(itemstack);
        return false;
    }
    ...
    public void fillContainerIfPossible() {
        if (this.fluidInput.get() == null) return;
        ItemStack copy = this.fluidInput.get()
            .copy();
        ItemStack filled = FluidContainerRegistry
            .fillFluidContainer(new FluidStack(FluidRegistry.LAVA, FluidContainerRegistry.BUCKET_VOLUME), copy);
        if (this.tank.getFluidAmount() >= FluidContainerRegistry.BUCKET_VOLUME) if (fluidOutput.add(filled, true)) {
    ...
    public void updateEntity() {
        ...
        while (canUse(EU_PER_LAVA_MB)) {
            ...
            if (this.tank.getCapacity() - this.tank.getFluidAmount() >= 1) {
                this.tank.setFluid(new FluidStack(FluidRegistry.LAVA, this.tank.getFluidAmount() + 1));
                used = true;
            }
            if (!used) break;
            else use(EU_PER_LAVA_MB);
        }
    }

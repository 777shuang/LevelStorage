...
public class LSKeyboardClient extends LSKeyboard implements ITickHandler {
    ...
    public LSKeyboardClient() {
        ...
        modBindings.put(RANGE_KEY_NAME, new KeyBinding(StatCollector.translateToLocal("key.range"), Keyboard.KEY_V));
        modBindings
            .put(RAY_SHOOT_KEY_NAME, new KeyBinding(StatCollector.translateToLocal("key.shoot"), Keyboard.KEY_R));
        modBindings.put(
            JETPACK_SWITCH_KEY_NAME,
            new KeyBinding(StatCollector.translateToLocal("key.jetpackSwitch"), Keyboard.KEY_F));
        // modBindings.put(ANTIMATTER_BOOTS_SPECIAL_FLIGHT, new KeyBinding(
        // StatCollector.translateToLocal("key.antimatterBoots"),
        // Keyboard.KEY_G));
        KeyBinding[] bsInternal = (KeyBinding[]) modBindings.values()
            .toArray(
                new KeyBinding[modBindings.values()
                    .size()]);
        KeyBindingRegistry.registerKeyBinding(new KeyBindingRegistry.KeyHandler(bsInternal) {
    ...
    public void initiateKeyChange(String id, EntityPlayer player, boolean isActive) {
    ...
    public void tickStart(EnumSet<TickType> type, Object... tickData) {
        if (!(tickData[0] instanceof EntityPlayer)) return;

...
public class ClientProxy extends CommonProxy {
    ...
    public static final String ENERGETIC_ENRICHED_MATTER_ORB_TEXTURE = getTexturePathFor(
        "itemEnergeticEnrichedMatterOrb");
    ...
    public static final ResourceLocation GUI_COMBUSTIBLE_GENERATOR = getResourceLocation(
        "gui/combustibleGenerator.png");
    ...
    public static CreativeTabs getCreativeTab(String name) {
        for (CreativeTabs t : CreativeTabs.creativeTabArray) {
            if (t.getTabLabel() == name) return t;
        }
        return null;
    }

    public int getArmorIndexFor(String forWhat) {
        if (forWhat == SUPERSONIC_DUMMY) return ARMOR_SUPERSONIC_RENDER_INDEX;
        else if (forWhat == ENH_LAPPACK_DUMMY) return ARMOR_ENHANCED_LAPPACK_RENDER_INDEX;
        return 0;
    }
    ...
    public void init() {
        ...
        ItemArmorEnhancedNanoChestplate.RENDER_ID = RenderingRegistry.addNewArmourRendererPrefix("enhnano");
        ...
        super.init();
        if (LevelStorage.configuration.get(LevelStorage.PERFORMANCE_CATEGORY, "enableArmorHUD", true)
            .getBoolean(true)) new ElectricHUD();
        ClientRegistry.bindTileEntitySpecialRenderer(TileEntityWirelessConductor.class, new WirelessConductorRender());
        MinecraftForgeClient
            .registerItemRenderer(LSBlockItemList.blockWlessConductor.blockID, new ItemWirelessConductorRender());
        MinecraftForgeClient
            .registerItemRenderer(LSBlockItemList.itemAtomicDisassembler.itemID, new ItemRendererAtomicDisassembler());
    ...
    public void messagePlayer(EntityPlayer player, String message, Object[] args) {
        Minecraft.getMinecraft().ingameGUI.getChatGUI()
            .addTranslatedMessage(message, args);
    }
}

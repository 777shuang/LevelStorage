...
public class XPStackRegistry {
    ...
    public void initCriticalNodes() {
        if (LevelStorage.configuration.get(LevelStorage.BALANCE_CATEGORY, "addCopperTinToBronzeCraftingRecipe", true)
            .getBoolean(true)) {
            ItemStack bronze = OreDictionary.getOres("ingotBronze")
                .get(0)
                .copy();
            bronze.stackSize = 4;
        ...
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.redstone), 4));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.netherQuartz), 1));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.ingotGold), 16));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.enderPearl), 192));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.glowstone), 8));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.diamond), 256));
        this.pushToRegistryWithConfig(new XPStack(new ItemStack(Item.netherStar), 8192));
    ...
    public boolean containsStack(ItemStack stack) {
        ...
        for (XPStack xpstack : stacks) {
            if (xpstack.stack.itemID == stack.itemID && xpstack.stack.getItemDamage() == stack.getItemDamage())
                return true;
        }
        return false;
    }
    ...
    public void pushToRegistry(XPStack stack) {
        FMLLog.log(
            Level.INFO,
            "Adding #" + stack.stack.itemID
                + ":"
                + stack.stack.getItemDamage()
                + " to the Xp Registry, value: "
                + stack.value);
        this.entries.add(stack);
    }

    public void pushToRegistryWithConfig(XPStack stack) {
        Property property = LevelStorage.configuration.get(
            XP_REGISTRY_CATEGORY,
            stack.stack.getItem()
                .getUnlocalizedName()
                .replace("item.", "")
                .replace(".name", "")
        ...
        if (value == -1) {
            LogHelper.warning("XP entry for item " + CommonHelper.getNiceStackName(stack.stack) + " is disabled");
            return;
        }
        FMLLog.log(
            Level.INFO,

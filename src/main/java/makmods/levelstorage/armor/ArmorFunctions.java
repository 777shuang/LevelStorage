...
public class ArmorFunctions {
    ...
    public static void bootsSpecialFly(EntityPlayer ep, World w, ItemStack armor) {
        ...
        if (!LSKeyboard.getInstance()
            .isKeyDown(ep, LSKeyboard.ANTIMATTER_BOOTS_SPECIAL_FLIGHT)) return;
        if (!ElectricItem.manager.canUse(armor, SPECIAL_FLIGHT_EU_COST)) return;
        if (!w.isRemote) ElectricItem.manager.discharge(armor, SPECIAL_FLIGHT_EU_COST, Integer.MAX_VALUE, true, false);
        ...
        y /= 4;
        z /= 4;
        if (Keys.instance.isBoostKeyDown(ep) && ElectricItem.manager.canUse(armor, SPECIAL_FLIGHT_EU_COST * 4)) {
            if (!w.isRemote)
                ElectricItem.manager.discharge(armor, SPECIAL_FLIGHT_EU_COST * 4, Integer.MAX_VALUE, true, false);
            x *= 2;
            y *= 2;
            z *= 2;
        }
        ep.motionX += x;
        if (y > 0 && ep.motionY < 0) ep.motionY = 0;
        ep.motionY += y;
        ep.motionZ += z;
    }
    ...
    public static MovingObjectPosition getMovingObjectPositionFromPlayer(World par1World, EntityPlayer par2EntityPlayer,
        ...
        Vec3 var13 = par1World.getWorldVec3Pool()
            .getVecFromPool(var7, var9, var11);
    ...
    public static void antimatterLeggingsFunctions(World world, EntityPlayer player, ItemStack armor) {
        ...
        if (LSKeyboard.getInstance()
            .isKeyDown(player, LSKeyboard.RAY_SHOOT_KEY_NAME) && player.isSneaking()) {
            int x = 0, y = 0, z = 0;
            ...
            if (mop != null && mop.typeOfHit == EnumMovingObjectType.TILE) {
                if (ElectricItem.manager.canUse(armor, ItemArmorAntimatterBase.EU_PER_TELEPORT)) {
                    ElectricItem.manager
                        .discharge(armor, ItemArmorAntimatterBase.EU_PER_TELEPORT, Integer.MAX_VALUE, true, false);
                }
                x = mop.blockX;
                y = mop.blockY;
                z = mop.blockZ;
                ...
                if (!world.isRemote) {
                    ...
                    if (toShoot != null) {
                        ...
                        PacketTeslaRay.issue(
                            ...
                            toShoot.posY,
                            toShoot.posZ);
                        toShoot.attackEntityFrom(
                            LSDamageSource.teslaRay,
                            world.rand.nextInt(5) + world.rand.nextInt(6) + world.rand.nextInt(12) + 2);
                    }
                }
    ...
    public static void speedUp(EntityPlayer player, ItemStack itemStack) {
        if (!speedTickerMap.containsKey(player)) speedTickerMap.put(player, 0);
        float speed = 0.66F;
        if ((ElectricItem.manager.canUse(itemStack, 1000)) && ((player.onGround) || (player.isInWater()))
            ...
            if (player.isInWater()) {
                speed = 0.1F;
                if (Keys.instance.isJumpKeyDown(player)) player.motionY += 0.1000000014901161D;
            }

            if (speed > 0.0F) player.moveFlying(0.0F, 1.0F, speed);
        }
    }
    ...
    public static void jumpBooster(World world, EntityPlayer player, ItemStack itemStack) {
        ...
        if (!onGroundMap.containsKey(player)) onGroundMap.put(player, true);
        if (!world.isRemote) {
            ...
            if ((ElectricItem.manager.canUse(itemStack, 4000)) && (player.onGround)) jumpCharge = 2.0F;
    ...
    public static void fly(int energy, EntityPlayer player, ItemStack itemStack, World world) {
        if (ElectricItem.manager.canUse(itemStack, energy)) {
            ...
            FlightRegistry.instance.modEnabledFlights.put(Reference.MOD_ID, new Flight(player, true));
            ...
            FlightRegistry.instance.modEnabledFlights.clear();
            FlightRegistry.instance.modEnabledFlights.put(Reference.MOD_ID, new Flight(player, false));
        }
    }

...
public class ItemArmorEnhancedNanoChestplate extends ItemArmor
    implements ISpecialArmor, IMetalArmor, IElectricItem, IHasRecipe {
    ...
    public void addCraftingRecipe() {
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemArmorEnhancedNanoChestplate),
            "clc",
            "qnq",
            "aja",
            Character.valueOf('c'),
            ...
            Character.valueOf('n'),
            Items.getItem("nanoBodyarmor"),
            Character.valueOf('l'),
            new ItemStack(LSBlockItemList.itemEnhLappack),
            Character.valueOf('j'),
            Items.getItem("electricJetpack")
                .copy(),
            Character.valueOf('a'),
    ...
    public void onArmorTickUpdate(World world, EntityPlayer player, ItemStack itemStack) {
        ...
        if (LevelStorage.isSimulating()) if (LSKeyboard.getInstance()
            .isKeyDown(player, LSKeyboard.JETPACK_SWITCH_KEY_NAME)) {
                if (Cooldownable.use(itemStack, COOLDOWN)) {
                    ...
                    CommonHelper.messagePlayer(
                        ...
                        EnumChatFormatting.GOLD + StatCollector.translateToLocalFormatted(
                            ...
                            NBTHelper.getBoolean(itemStack, ON_NBT) ? StatCollector.translateToLocal("jetpack.on")
                                : StatCollector.translateToLocal("jetpack.off")));
                }
            }
        if (NBTHelper.getBoolean(itemStack, ON_NBT)) {
            ...
            if ((Keys.instance.isJumpKeyDown(player)) && (Keys.instance.isModeSwitchKeyDown(player))
                && (toggleTimer == 0)) {
                toggleTimer = 10;
                hoverMode = !hoverMode;
                ...
                if (LevelStorage.isSimulating()) {
                    ...
                    if (hoverMode)
                        CommonHelper.messagePlayer(player, EnumChatFormatting.DARK_GREEN + "Hover Mode enabled.");
                    else {
                        CommonHelper.messagePlayer(player, EnumChatFormatting.DARK_RED + "Hover Mode disabled.");
                    }
                }
            }

            if ((Keys.instance.isJumpKeyDown(player)) || ((hoverMode) && (player.motionY < -0.3499999940395355D))) {
                if (ElectricItem.manager.canUse(itemStack, ENERGY_FLYING_PER_TICK)) {
            ...
            if (jetpackUsed) {
                ElectricItem.manager.use(itemStack, ENERGY_FLYING_PER_TICK, player);
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ENHANCED_NANO_CHESTPLATE_TEXTURE);
    }

...
public class ItemArmorLevitationBoots extends ItemArmor
    implements ISpecialArmor, IMetalArmor, IElectricItem, IFlyArmor, IHasRecipe {
    ...
    public ItemArmorLevitationBoots(int id) {
        super(id, EnumArmorMaterial.DIAMOND, LevelStorage.proxy.getArmorIndexFor(CommonProxy.SUPERSONIC_DUMMY), 3);
    ...
    public void onEntityLivingFallEvent(LivingFallEvent event) {
        if ((!event.entityLiving.worldObj.isRemote) && ((event.entity instanceof EntityLivingBase))) {
    ...
    public static boolean checkPlayer(EntityPlayer p) {
        ...
        for (ItemStack st : inv.armorInventory) {
            if (st != null && st.getItem() instanceof IFlyArmor)
                if (((IFlyArmor) st.getItem()).isFlyArmor(st)) isFound = true;
        }

        for (ItemStack st : inv.mainInventory) {
            if (st != null) if (st.getItem() instanceof ItemQuantumRing) isFound = true;
        }
        ...
        if (!isFound) {
            if (!p.worldObj.isRemote) {
                ...
                PacketDispatcher.sendPacketToPlayer(PacketTypeHandler.populatePacket(flUpd), (Player) p);
                return false;
            }
        } else {
            return true;
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.LEVITATION_BOOTS_TEXTURE);
    }

...
public class ItemArmorAntimatterBase extends ItemArmor implements ISpecialArmor, IElectricItem {
    ...
    public void onArmorTickUpdate(World world, EntityPlayer player, ItemStack itemStack) {
        if (this.armorType == CHESTPLATE) ArmorFunctions.extinguish(player, world);
            ...
            ArmorFunctions.helmetFunctions(
                world,
                player,
                itemStack,
    ...
    public double getDamageAbsorptionRatio() {
        if (this.armorType == CHESTPLATE) return 1.1D;
        return 1.0D;
    }
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        if (this.armorType == HELMET)
            this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ANTIMATTER_HELMET_TEXTURE);
        else if (this.armorType == CHESTPLATE)
            this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ANTIMATTER_CHESTPLATE_TEXTURE);
        else if (this.armorType == LEGGINGS)
            this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ANTIMATTER_LEGGINGS_TEXTURE);
        else if (this.armorType == BOOTS)
            this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ANTIMATTER_BOOTS_TEXTURE);
    ...
    public boolean canProvideEnergy(ItemStack itemStack) {
        if (this.armorType == CHESTPLATE) return true;
        else return false;
    }

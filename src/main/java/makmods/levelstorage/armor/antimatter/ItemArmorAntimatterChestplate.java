...
public class ItemArmorAntimatterChestplate extends ItemArmorAntimatterBase implements IHasRecipe {
    ...
    public void onUpdate(LivingUpdateEvent event) {
        ...
        for (EntityPlayer ep : playerList) {
            ...
            if (!(armor.getItem() instanceof ItemArmorAntimatterChestplate)) continue;
            int distance = CommonHelper.getDistance(
                ep.posX,
                ep.posY,
                ep.posZ,
            ...
            if (distance < KEEPAWAY_DISTANCE) {
                if (ElectricItem.manager.canUse(armor, ENERGY_PER_HIT)) {
                    if (!event.entityLiving.worldObj.isRemote) ElectricItem.manager.use(armor, ENERGY_PER_HIT, ep);
                    event.entityLiving.attackEntityFrom(LSDamageSource.forcefieldArmorInstaKill, 20.0F);
                }
            }
        }
    }
    ...
    public void onHurt(LivingHurtEvent event) {
        ...
        if (!(armor.getItem() instanceof ItemArmorAntimatterChestplate)) return;
        int toBeUsed = (int) (event.ammount * ItemArmorAntimatterBase.ENERGY_PER_DAMAGE);
        if (ElectricItem.manager.canUse(armor, toBeUsed)) {
            if (!event.entityLiving.worldObj.isRemote) ElectricItem.manager.use(armor, toBeUsed, ep);
    ...
    public void addCraftingRecipe() {
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemArmorAntimatterChestplate),
            "pcp",
            "pap",
            "pep",
            'p',
            ...
            'e',
            new ItemStack(LSBlockItemList.itemAntimatterCrystal),
            'a',

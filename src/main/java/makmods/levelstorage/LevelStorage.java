...
// "Forge@[9.10.0.804,);required-after:IC2")
@Mod(
    modid = Reference.MOD_ID,
    name = Reference.MOD_NAME,
    version = Reference.VERSION,
    dependencies = "required-after:IC2@[2.0,)")
@NetworkMod(
    channels = { Reference.MOD_ID, LSKeyboard.PACKET_KEYBOARD_CHANNEL, Reference.CUSTOM_PACKET_CHANNEL },
...
public class LevelStorage {
    ...
    @SidedProxy(
        clientSide = "makmods.levelstorage.proxy.ClientProxy",
        serverSide = "makmods.levelstorage.proxy.CommonProxy")
    public static CommonProxy proxy;

    @SidedProxy(
        clientSide = "makmods.levelstorage.proxy.LSKeyboardClient",
        serverSide = "makmods.levelstorage.proxy.LSKeyboard")
    ...
    public void preInit(FMLPreInitializationEvent event) {
        ...
        configuration.load();
        Config.ACTIVE = true;
        LevelStorage.itemLevelStorageBookSpace = config
            .get(Configuration.CATEGORY_GENERAL, "bookCapacity", 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2 * 2) // 16384
            .getInt();
    ...
    public void serverStarting(FMLServerStartingEvent event) {
        ...
        ((ServerCommandManager) MinecraftServer.getServer()
            .getCommandManager()).registerCommand(new CommandChargeItems());
    }
        ...
        for (IMCMessage message : messages) {
            try {
                if (message.isStringMessage()) {
                    ...
                    throw new LSIMCHandler.LSIMCException("Value must be string!");
                }
            } catch (Exception e) {
                LogHelper.warning("Mod " + message.getSender() + " sent an invalid FMLInterModComms message.");
                e.printStackTrace();
            }
        }
    }
    ...
    public void postInit(FMLPostInitializationEvent event) {
        ...
        proxy.postInit();
        LogHelper.info("Initialization took " + (System.currentTimeMillis() - initTimeMeter) + " ms.");

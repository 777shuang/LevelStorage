...
public class BlockLocation {
    ...
    public String getBlockName() {
        try {
            ...
            return new ItemStack(Block.blocksList[w.getBlockId(x, y, z)]).getDisplayName();
        } catch (Exception e) {
            return "Unknown.";
        }
    }
    ...
    public int getDistance(BlockLocation other) {
        if (this.dimId != other.dimId) return Integer.MAX_VALUE;
    ...
    public static BlockLocation readFromNBT(NBTTagCompound nbt) {
        ...
        if ((blockLocationC.getInteger(X_NBT) == 0) && (blockLocationC.getInteger(Y_NBT) == 0)
            && (blockLocationC.getInteger(Z_NBT) == 0)) return null;
    ...
    public static int getEnergyDiscount(int energy, int distance) {
        if (LevelStorage.configuration.get(Configuration.CATEGORY_GENERAL, "enableEnergyLoss", false)
            .getBoolean(false)) {
            // Cross-Dimensional
            if (distance == Integer.MAX_VALUE) return (int) (energy * 0.25f);
            // Distance < 1000
            if (distance < 1000) return (int) (energy * 0.05f);
            // Distance > 1000 <2000
            if (distance > 1000 && distance < 2000) return (int) (energy * 0.1f);
            // Distance > 2000
            if (distance > 2000) return (int) (energy * 0.15f);
        }
        return 0;
    }
    ...
    public TileEntity getTileEntity() {
        if (!isDimIdValid(this.dimId)) return null;

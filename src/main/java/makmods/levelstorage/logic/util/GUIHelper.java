...
public class GUIHelper {
    ...
    // it.
    protected static boolean mergeItemStack(Container c, ItemStack par1ItemStack, int par2, int par3, boolean par4) {
        boolean flag1 = false;
        int k = par2;
    ...
    public static ItemStack shiftClickSlot(Container c, EntityPlayer player, int sourceSlotIndex) {
        ...
        if (sourceSlot != null && sourceSlot.getHasStack()) {
            ...
            if (sourceSlot.inventory == player.inventory) {
                ...
                for (run = 0; run < 2 && sourceItemStack.stackSize > 0; ++run) {
                    ListIterator var9 = c.inventorySlots.listIterator(c.inventorySlots.size());

                    while (var9.hasPrevious()) {
                        targetSlot = (Slot) var9.previous();
                        if (targetSlot.inventory == player.inventory && targetSlot.isItemValid(sourceItemStack)
                            && (targetSlot.getStack() != null || run == 1)) {
                            mergeItemStack(c, sourceItemStack, targetSlot.slotNumber, targetSlot.slotNumber + 1, false);
                            if (sourceItemStack.stackSize == 0) {
                                break;
                            }
                        }

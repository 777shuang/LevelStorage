...
public class NBTHelper {
    ...
    public static class Cooldownable {
        ...
        public static void onUpdate(ItemStack stack, int maxCooldown) {
            ...
            if (!NBTHelper.verifyKey(stack, COOLDOWN_NBT)) NBTHelper.setInteger(stack, COOLDOWN_NBT, maxCooldown);
            if (NBTHelper.getInteger(stack, COOLDOWN_NBT) > 0) NBTHelper.decreaseInteger(stack, COOLDOWN_NBT, 1);
        }
        ...
        public static boolean use(ItemStack stack, int maxCooldown) {
            NBTHelper.checkNBT(stack);
            if (NBTHelper.getInteger(stack, COOLDOWN_NBT) > 0) return false;
            else {

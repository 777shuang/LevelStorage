...
public class AdvBlockFinder {
    ...
    private boolean isBlockOreDict(Block bl) {
        return bl != null && targetName != null ? OreDictionary.getOreName(OreDictionary.getOreID(new ItemStack(bl)))
            .equals(targetName) : false;
    }
    ...
    private void findContinuation(BlockLocation loc) {
        ...
        if (loc != null) {
            for (BlockLocation locat : blocksFound) {
                if (locat.equals(loc)) found = true;
            }
            if (!found) blocksFound.add(loc);
        }
        if (!found) {
            for (int dir = 0; dir < 6; dir++) {
                ...
                Block currBlock = Block.blocksList[this.world.getBlockId(currX, currY, currZ)];
                if (currBlock != null) {

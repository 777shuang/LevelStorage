...
public class TranslocationHelper {
    ...
    public static Entity teleportEntity(World newworld, Entity entity, int dimension, ChunkCoordinates spawn,
        float yaw) {
        ...
        entity.setLocationAndAngles(spawn.posX + 0.5D, spawn.posY, spawn.posZ + 0.5D, yaw, entity.rotationPitch);
        ...
        if ((entity instanceof EntityPlayerMP)) {
            EntityPlayerMP player = (EntityPlayerMP) entity;
            if (changingworlds) player.mcServer.getConfigurationManager()
                .func_72375_a(player, (WorldServer) newworld);
        ...
        if (((entity instanceof EntityPlayerMP)) && (changingworlds)) {
            ...
            player.mcServer.getConfigurationManager()
                .updateTimeAndWeatherForPlayer(player, (WorldServer) newworld);
            player.mcServer.getConfigurationManager()
                .syncPlayerInventory(player);
            Iterator iter = player.getActivePotionEffects()
                .iterator();
            ...
            player.playerNetServerHandler.sendPacketToPlayer(
                new Packet43Experience(player.experience, player.experienceTotal, player.experienceLevel));
        }
        entity.setLocationAndAngles(spawn.posX + 0.5D, spawn.posY, spawn.posZ + 0.5D, yaw, entity.rotationPitch);
        if (mount != null) {
            if ((entity instanceof EntityPlayerMP)) {
    ...
    private static void removeEntityFromWorld(World world, Entity entity) {
        if ((entity instanceof EntityPlayer)) {
            ...
            if ((entity.addedToChunk) && (world.getChunkProvider()
                .chunkExists(i, j))) {
            ...
            for (int l1 = i1; l1 < j1; l1++) {
                if (world.blockExists(k1, 64, l1)) {
                    for (int i2 = k - 1; i2 < l; i2++) {
                        ...
                        if (block != null) block
                            .addCollisionBoxesToList(world, k1, i2, l1, axisalignedbb, collidingBoundingBoxes, entity);
                    }
                }
            }
        }

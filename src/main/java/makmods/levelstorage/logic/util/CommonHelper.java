...
public class CommonHelper {
    ...
    public static SyncType invertType(SyncType type) {
        if (type == SyncType.RECEIVER) return SyncType.TRANSMITTER;
        if (type == SyncType.TRANSMITTER) return SyncType.RECEIVER;
        return null;
    }
    ...
    public static boolean compareStacksGenerally(ItemStack s1, ItemStack s2) {
        ...
        return (s1.itemID == s2.itemID) && (s1.getItemDamage() == s2.getItemDamage()) && (s1.stackSize == s2.stackSize);
    }

    public static boolean areStacksEqual(ItemStack first, ItemStack second) {
        if (first == null || second == null) return false;
        return first.itemID == second.itemID
            && (first.getItemDamage() == second.getItemDamage() || first.getItemDamage() == OreDictionary.WILDCARD_VALUE
    ...
    public static ItemStack createEnchantedBook(Enchantment ench, int level) {
        ...
        book.getTagCompound()
            .setTag("StoredEnchantments", storedEnchs);
        return book;
    }

    public static void dropBlockInWorld(World par1World, int par2, int par3, int par4, ItemStack par5ItemStack) {
     ...
     */
    public static void dropBlockInWorld_exact(World world, double x, double y, double z, ItemStack stack) {
    ...
    public static boolean handleMachineRightclick(World world, int x, int y, int z, EntityPlayer player) {
    ...
    public static boolean compareStacksGenerallyNoStackSize(ItemStack s1, ItemStack s2) {
        if (s1 == null && s2 == null) return true;
        if (s1 == null) return false;
        if (s2 == null) return false;
    ...
    public static int getRandomNumber(int signs) {
        ...
        if (negativeSign) return (int) (-(Math.random() * maxSign));
        else return (int) (Math.random() * maxSign);
    }
            ...
            if (FMLCommonHandler.instance()
                ...
                sb.append(
                    LanguageRegistry.instance()
                        .getStringLocalization(name));
            else sb.append(name);
        } else sb.append(stack.getDisplayName());
        return sb.toString();
    }
    ...
    public static int getDistance(double startX, double startY, double startZ, double endX, double endY, double endZ) {
    ...
    public static int getDistanceFloor(double startX, double startY, double startZ, double endX, double endY,
        double endZ) {
        double x = endX - startX;
        double y = endY - startY;
        double z = endZ - startZ;
    ...
    public static int getDistanceCeil(double startX, double startY, double startZ, double endX, double endY,
        double endZ) {
        double x = endX - startX;
        double y = endY - startY;
        double z = endZ - startZ;

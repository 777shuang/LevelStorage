...
public class ExperienceRecipe implements IRecipe {
    ...
    public ItemStack getResInternal(InventoryCrafting inventoryCrafting) {
            ...
            if (currentStack != null && !currentStack.equals(null)) {
                if (currentStack.getItem() instanceof ItemXPTome) {
                    if (currentStack.stackTagCompound != null) {
                        if (currentStack.stackTagCompound.hasKey(ItemXPTome.STORED_XP_NBT)) {
                            if (seenBook) return null;
                            initialBookStack = currentStack;
                            bookXp = currentStack.stackTagCompound.getInteger(ItemXPTome.STORED_XP_NBT);
                            seenBook = true;
                            cycleCompleted = true;
                        }
                    }
        ...
        if ((bookXp + buffXp) >= LevelStorage.itemLevelStorageBookSpace) return null;
        ...
        result.stackTagCompound.setInteger(ItemXPTome.STORED_XP_NBT, totalXp);

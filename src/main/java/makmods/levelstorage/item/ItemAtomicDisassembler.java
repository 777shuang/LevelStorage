...
public class ItemAtomicDisassembler extends Item implements IElectricItem, IChargeable, IHasRecipe {
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.DESTRUCTOR_TEXTURE);
    }
    ...
    static {
        DEAL_DAMAGE = LevelStorage.configuration
            .get(Configuration.CATEGORY_GENERAL, "atomicDisassemblersEnableDamage", true)
            .getBoolean(true);
    }
    ...
    public void changeCharge(ItemStack itemStack, World world, EntityPlayer player) {
        ...
        if (initialCharge != getChargeFor(itemStack)) LevelStorage.proxy
            .messagePlayer(player, "Tunnel length: " + (int) Math.pow(2, getChargeFor(itemStack)), new Object[0]);
    }
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (LevelStorage.isSimulating()) {
            ...
            if (DEAL_DAMAGE) player.attackEntityFrom(LSDamageSource.disassembled, damage);
            List<ParticleInternal> toSend = Lists.newArrayList();
            for (int curr = 0; curr < length; curr++) {

                BlockLocation loc = new BlockLocation(x, y, z).move(hitFrom.getOpposite(), curr);
    ...
    public void mineThreeByThree(ItemStack device, ForgeDirection hitFrom, BlockLocation currBlock, World par2World,
        EntityPlayer player, List<ParticleInternal> particles) {
        ...
        if (!ElectricItem.manager.canUse(device, ENERGY_USE_BASE)) return;
        for (BlockLocation blockLoc : removeBlocks) {
            if (blockLoc != null) {
                Block b = Block.blocksList[par2World.getBlockId(blockLoc.getX(), blockLoc.getY(), blockLoc.getZ())];
                int aimBlockMeta = par2World.getBlockMetadata(blockLoc.getX(), blockLoc.getY(), blockLoc.getZ());
                if (b != null) {
                    if (b.getBlockHardness(par2World, blockLoc.getX(), blockLoc.getY(), blockLoc.getZ()) > 0.0F) {
                        if (b.removeBlockByPlayer(
                            ...
                            for (ItemStack drop : drops) {
                                ...
                                particle.name = "tilecrack_" + b.blockID + "_" + aimBlockMeta;
    ...
    public void onUpdate(ItemStack par1ItemStack, World par2World, Entity par3Entity, int par4, boolean par5) {
        if (!par2World.isRemote) {
            if (!(par3Entity instanceof EntityPlayerMP)) return;
            ...
            if (LSKeyboard.getInstance()
                .isKeyDown(player, LSKeyboard.RANGE_KEY_NAME)) {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        String[] lines = StatCollector.translateToLocal("tooltip.atomicDisassembler")
            .split("\n");
    ...
    public void addCraftingRecipe() {
        ...
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemAtomicDisassembler),
            "ccc",
            "lda",
            "ccc",
            Character.valueOf('c'),
            IC2Items.CARBON_PLATE,
            Character.valueOf('l'),
            Items.getItem("miningLaser"),
            Character.valueOf('d'),
            new ItemStack(LSBlockItemList.itemEnhDiamondDrill),
            Character.valueOf('a'),
            IC2Items.ADV_CIRCUIT);
    }

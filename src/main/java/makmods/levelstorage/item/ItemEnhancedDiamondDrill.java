...
public class ItemEnhancedDiamondDrill extends ItemPickaxe implements IElectricItem, IHasRecipe {
    ...
    public static enum EnhancementUtility {
        ...
        public static EnhancementUtility readDrillEnhancement(ItemStack stack) {
            ...
            if (!(stack.getItem() instanceof ItemEnhancedDiamondDrill)) return null;
            if (stack.getTagCompound() == null) return null;
            NBTHelper.checkNBT(stack);
            if (!stack.getTagCompound()
                .hasKey(ENHANCEMENT_NBT)) return null;
            NBTTagCompound enhNBT = stack.getTagCompound()
                .getCompoundTag(ENHANCEMENT_NBT);
            if (!enhNBT.hasKey(ENHANCEMENT_ID_NBT) || !enhNBT.hasKey(ENHANCEMENT_LVL_NBT)) return null;
            int id = enhNBT.getInteger(ENHANCEMENT_ID_NBT);
            int lvl = enhNBT.getInteger(ENHANCEMENT_LVL_NBT);
            if (id == Enchantment.silkTouch.effectId) return SILKTOUCH;
            else if (id == Enchantment.fortune.effectId) {
                if (lvl == 1) return FORTUNE_1;
                else if (lvl == 2) return FORTUNE_2;
                else if (lvl == 3) return FORTUNE_3;
            } else return UNKNOWN;
            return null;
        }
    ...
    static {
        Property p = LevelStorage.configuration.get(Configuration.CATEGORY_GENERAL, "enhancedDiamondDrillSpeed", 32);
    ...
    public static enum Mode {
        ...
        TUNNEL("3x3x1 (width x height x depth)"),
        NORMAL("normal (1x)");
    ...
    public static void invertMode(ItemStack stack, EntityPlayer player) {
        try {
            ...
            LevelStorage.proxy.messagePlayer(
                player,
                "Drill mode: " + Mode.values()[NBTHelper.getInteger(stack, MODE_NBT)].name,
                new Object[0]);
        } catch (Exception e) {
            System.out.println(Reference.MOD_NAME + ": something went wrong when tried to set drill's mode.");
            e.printStackTrace();
        }
    }
    ...
    public void onUpdate(ItemStack par1ItemStack, World par2World, Entity par3Entity, int par4, boolean par5) {
        if (!par2World.isRemote) {
            if (!NBTHelper.verifyKey(par1ItemStack, MODE_NBT))
                NBTHelper.setInteger(par1ItemStack, MODE_NBT, Mode.NORMAL.ordinal());
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (!par2World.isRemote) {
            if (Keys.instance.isModeSwitchKeyDown(par3EntityPlayer)) {
                if (!Cooldownable.use(par1ItemStack, COOLDOWN)) return par1ItemStack;
    ...
    public void addCraftingRecipe() {
        ...
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemEnhDiamondDrill),
            "cdc",
            "did",
            "aea",
            Character.valueOf('c'),
            ...
            Character.valueOf('e'),
            Items.getItem("advBattery"),
            Character.valueOf('i'),
            Items.getItem("diamondDrill"),
            Character.valueOf('a'),
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        NBTTagCompound enh = par1ItemStack.stackTagCompound.getCompoundTag(ENHANCEMENT_NBT);
    ...
    public void getSubItems(int par1, CreativeTabs par2CreativeTabs, List par3List) {
        ...
        int[][] enhancements = { { Enchantment.silkTouch.effectId, 1 }, { Enchantment.fortune.effectId, 3 },
            { Enchantment.fortune.effectId, 2 }, { Enchantment.fortune.effectId, 1 } };
    ...
    public boolean onBlockDestroyed(ItemStack par1ItemStack, World par2World, int par3, int par4, int par5, int par6,
        ...
        if (!par2World.isRemote) {
            ...
            if (enh.hasKey(ENHANCEMENT_ID_NBT)) {
                ...
                int lvl = enh.getInteger(ENHANCEMENT_LVL_NBT);
                if (encid > 0 && lvl > 0) {
                    if (Enchantment.enchantmentsList[encid] == Enchantment.fortune) fortune = lvl;
                    if (Enchantment.enchantmentsList[encid] == Enchantment.silkTouch) silktouch = true;
                }
            }
            if (ElectricItem.manager.canUse(par1ItemStack, ENERGY_PER_USE)) {
                ElectricItem.manager.use(par1ItemStack, ENERGY_PER_USE, par7EntityLivingBase);
            }
            ...
            if (name.startsWith("ore")) {
                AdvBlockFinder finder = new AdvBlockFinder(par2World, par4, par5, par6, name);
                for (BlockLocation oreCh : finder.getBlocksFound()) {
                    int blockId = par2World.getBlockId(oreCh.getX(), oreCh.getY(), oreCh.getZ());

                    int blockMeta = par2World.getBlockMetadata(oreCh.getX(), oreCh.getY(), oreCh.getZ());
                    Block b = Block.blocksList[blockId];
                    if (b != null) {
                        if (par7EntityLivingBase instanceof EntityPlayer) {
                            if (b.getBlockHardness(par2World, oreCh.getX(), oreCh.getY(), oreCh.getZ()) != -1.0F) {
                ...
                if (NBTHelper.getInteger(par1ItemStack, MODE_NBT) == Mode.TUNNEL.ordinal()) {
                    if (par7EntityLivingBase instanceof EntityPlayer) {
                        EntityPlayer player = (EntityPlayer) par7EntityLivingBase;
                        if (!player.isSneaking()) {
                            MovingObjectPosition pos = this.getMovingObjectPositionFromPlayer(par2World, player, true);
                            if (pos != null) if (pos.typeOfHit == EnumMovingObjectType.TILE) {
                                ...
                                switch (hitFrom) {
                                    case DOWN: {
                                        ...
                                        removeBlocks[4] = currBlock.move(ForgeDirection.NORTH, 1)
                                            .move(ForgeDirection.EAST, 1);
                                        removeBlocks[5] = currBlock.move(ForgeDirection.WEST, 1)
                                            .move(ForgeDirection.NORTH, 1);
                                        removeBlocks[6] = currBlock.move(ForgeDirection.NORTH, 1)
                                            .move(ForgeDirection.WEST, 1);
                                        removeBlocks[7] = currBlock.move(ForgeDirection.WEST, 1)
                                            .move(ForgeDirection.SOUTH, 1);
                                        removeBlocks[8] = currBlock.move(ForgeDirection.SOUTH, 1)
                                            .move(ForgeDirection.EAST, 1);
                                        break;
                                    }
                                    case UP: {
                                    ...
                                    case NORTH: {
                                        ...
                                        removeBlocks[3] = currBlock.move(ForgeDirection.EAST, 1);
                                        // Up-west & Down-west
                                        removeBlocks[4] = currBlock.move(ForgeDirection.DOWN, 1)
                                            .move(ForgeDirection.WEST, 1);
                                        removeBlocks[5] = currBlock.move(ForgeDirection.UP, 1)
                                            .move(ForgeDirection.EAST, 1);

                                        removeBlocks[6] = currBlock.move(ForgeDirection.UP, 1)
                                            .move(ForgeDirection.WEST, 1);
                                        removeBlocks[7] = currBlock.move(ForgeDirection.DOWN, 1)
                                            .move(ForgeDirection.EAST, 1);

                                        removeBlocks[8] = currBlock.move(ForgeDirection.UP, 1)
                                            .move(ForgeDirection.EAST, 1);
                                        removeBlocks[9] = currBlock.move(ForgeDirection.DOWN, 1)
                                            .move(ForgeDirection.WEST, 1);
                                    ...
                                    case WEST: {
                                        ...
                                        removeBlocks[4] = currBlock.move(ForgeDirection.UP, 1)
                                            .move(ForgeDirection.NORTH, 1);
                                        removeBlocks[5] = currBlock.move(ForgeDirection.DOWN, 1)
                                            .move(ForgeDirection.SOUTH, 1);
                                ...
                                for (BlockLocation blockLoc : removeBlocks) {
                                    if (blockLoc != null) {
                                        Block b = Block.blocksList[par2World
                                            .getBlockId(blockLoc.getX(), blockLoc.getY(), blockLoc.getZ())];
                                        int aimBlockMeta = par2World
                                            .getBlockMetadata(blockLoc.getX(), blockLoc.getY(), blockLoc.getZ());
                                        if (b != null) {
                                            if (b.getBlockHardness(
                                                ...
                                                if (b.removeBlockByPlayer(
                                                    ...
                                                    if (!silktouch) {
                                                        ...
                                                        if (b.canSilkHarvest(
                                                            ...
                                                            blockLoc.getZ(),
                                                            aimBlockMeta)) {
                                                            ItemStack itemstack = new ItemStack(b, 1, aimBlockMeta);
                                                            if (itemstack != null) {
                                                                this.dropBlockAsItem_do(
                                                                    par2World,
                                                                    blockLoc.getX(),
                                                ...
                                                if (ElectricItem.manager.canUse(par1ItemStack, ENERGY_PER_USE)) {
                                                    ElectricItem.manager
                                                        .use(par1ItemStack, ENERGY_PER_USE, par7EntityLivingBase);
                                                } else {
                                                    break;
                                                }
                                            }
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.iconPass1 = par1IconRegister.registerIcon(ClientProxy.ITEM_ENHANCED_DIAMOND_DRILL_PASS_ONE);
        this.iconPass2 = par1IconRegister.registerIcon(ClientProxy.ITEM_ENHANCED_DIAMOND_DRILL_PASS_TWO);
    }

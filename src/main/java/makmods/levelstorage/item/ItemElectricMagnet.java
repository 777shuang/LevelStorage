...
public class ItemElectricMagnet extends Item implements IElectricItem, IHasRecipe {
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ELECTRIC_MAGNET_TEXTURE);
    }
                ...
                if (ElectricItem.manager.canUse(par1ItemStack, ENERGY_PER_TICK)) {
                    ...
                    for (Object obj : par2World.loadedEntityList) {
                        if (obj instanceof EntityItem || obj instanceof EntityXPOrb) {
                            ...
                            double distanceX = Math.abs(par3Entity.posX - item.posX);
                            double distanceY = Math.abs(par3Entity.posY - item.posY);
                            double distanceZ = Math.abs(par3Entity.posZ - item.posZ);
                            double distanceTotal = distanceX + distanceY + distanceZ;
                            if (distanceTotal < RANGE) {
                                used = true;
                                item.setPosition(par3Entity.posX, par3Entity.posY, par3Entity.posZ);
                            }
                        }
                    }
                    if (par3Entity instanceof EntityLivingBase) if (used)
                        ElectricItem.manager.use(par1ItemStack, ENERGY_PER_TICK, (EntityLivingBase) par3Entity);
                }
            }
        }
    }
    ...
    public String getStringState(ItemStack par1ItemStack) {
        return "Active: "
            + (SimpleMode.readFromNBT(par1ItemStack.stackTagCompound).boolValue ? EnumChatFormatting.DARK_GREEN + "yes"
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (!par2World.isRemote) {
            writeInitialMode(par1ItemStack);
            SimpleMode.readFromNBT(par1ItemStack.stackTagCompound)
                .getReverse()
                .writeToNBT(par1ItemStack.stackTagCompound);
            LevelStorage.proxy.messagePlayer(par3EntityPlayer, getStringState(par1ItemStack), new Object[0]);
        }
    ...
    public void addCraftingRecipe() {
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemElectricMagnet),
            "cc ",
            "cic",
            " cb",
            Character.valueOf('c'),

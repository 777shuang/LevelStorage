...
public class ItemAntimatterRing extends ItemQuantumRing {
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ANTIMATTER_RING_TEXTURE);
    }

    @Override
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        par3List.add(EnumChatFormatting.DARK_PURPLE + StatCollector.translateToLocal("tooltip.antimatterring"));
    }
    ...
    public void onUpdate(ItemStack itemStack, World world, Entity par3Entity, int par4, boolean par5) {
        ...
        for (ItemStack device : inv.armorInventory) if (chargeItem(device, itemStack)) {
            didSomething = true;
            break;
        }
        if (!didSomething) {
            for (ItemStack device : inv.mainInventory) {
                if (device != null) if (device.getItem() instanceof ItemQuantumRing) continue;
        ...
        ArmorFunctions.fly(ItemArmorLevitationBoots.FLYING_ENERGY_PER_TICK, player, itemStack, world);
        ...
        ArmorFunctions
            .helmetFunctions(world, player, itemStack, ItemArmorTeslaHelmet.RAY_COST, ItemArmorTeslaHelmet.FOOD_COST);
    }

    public static ItemStack getArmorInternal(String s) {
        if (s.equals("h")) return new ItemStack(LSBlockItemList.itemArmorAntimatterHelmet);
        else if (s.equals("c")) return new ItemStack(LSBlockItemList.itemArmorAntimatterChestplate);
        else if (s.equals("l")) return new ItemStack(LSBlockItemList.itemArmorAntimatterLeggings);
        else if (s.equals("b")) return new ItemStack(LSBlockItemList.itemArmorAntimatterBoots);
        else throw new RuntimeException("getArmorInternal(): wow, this mustn't happen!!");
    }
            ...
            for (String s2 : lst) {
                for (String s3 : lst) {
                    for (String s4 : lst) {
                        ...
                        if (curList.contains(lst.get(0)) && curList.contains(lst.get(1))
                            ...
                            Recipes.advRecipes.addRecipe(
                                ...
                                'Q',
                                new ItemStack(LSBlockItemList.itemQuantumRing),
                                'A',
                                armor1,
                                'B',
                                armor2,

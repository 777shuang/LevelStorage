...
public class ItemElectricLighter extends ItemBasicElectric implements IHasRecipe {
    ...
    public boolean addFire(World world, int x, int y, int z, int sideHit) {
        ...
        BlockLocation shifted = currBlockLocation.move(ForgeDirection.getOrientation(sideHit), 1);
        if (!world.isAirBlock(shifted.getX(), shifted.getY(), shifted.getZ())) {
            return false;
        }
        world.setBlock(shifted.getX(), shifted.getY(), shifted.getZ(), Block.fire.blockID);
        addedFire = true;
        return addedFire;
    }
    ...
    public void addCraftingRecipe() {
        Recipes.advRecipes.addShapelessRecipe(
            new ItemStack(LSBlockItemList.itemElectricLighter),
    ...
    public boolean onBlockClick(ItemStack item, World world, EntityPlayer player, int x, int y, int z, int side) {
        ...
        if (blockID == 0) return addFire(world, x, y, z, side);
        int blockMeta = world.getBlockMetadata(x, y, z);
        ItemStack smResult = FurnaceRecipes.smelting()
            .getSmeltingResult(new ItemStack(blockID, 1, blockMeta));
        if (smResult == null) return addFire(world, x, y, z, side);
        if (smResult.getItem() instanceof ItemBlock)
            world.setBlock(x, y, z, smResult.itemID, smResult.getItemDamage(), 3);
        else return addFire(world, x, y, z, side);
        return true;
    }

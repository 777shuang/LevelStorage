...
public abstract class ItemBasicElectric extends Item implements IElectricItem {
     ...
     */
    public abstract boolean onBlockClick(ItemStack item, World world, EntityPlayer player, int x, int y, int z,
        int side);
     ...
     * @return whether or not {@link #energyPerUse} should be consumed
     */
    public abstract boolean onRightClick(ItemStack item, World world, EntityPlayer player);

    public ItemBasicElectric(int id, int tier, int storage, int transferLimit, int energyPerUse) {
        super(id);
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (!par2World.isRemote) if (ElectricItem.manager.canUse(par1ItemStack, energyPerUse))
            if (onRightClick(par1ItemStack, par2World, par3EntityPlayer))
                ElectricItem.manager.use(par1ItemStack, energyPerUse, par3EntityPlayer);
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int x, int y,
        ...
        if (!par3World.isRemote) if (ElectricItem.manager.canUse(par1ItemStack, energyPerUse))
            if (onBlockClick(par1ItemStack, par3World, par2EntityPlayer, x, y, z, side)) {
                ElectricItem.manager.use(par1ItemStack, energyPerUse, par2EntityPlayer);
                return false;
            }
        return true;
    }

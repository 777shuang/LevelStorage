...
public class ItemAdvancedScanner extends Item implements IElectricItem, IHasRecipe {
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        String[] lines = StatCollector.translateToLocal("tooltip.advScanner")
            .split("\n");
    ...
    public ItemStack onItemRightClick(ItemStack par1ItemStack, World par2World, EntityPlayer par3EntityPlayer) {
        if (!par2World.isRemote) {
            ...
            if (!(getNBTInt(par1ItemStack, NBT_COOLDOWN) == 0)) return par1ItemStack;
                ...
                for (int x = -(RADIUS / 2); x < (RADIUS / 2); x++) {
                    for (int z = -(RADIUS / 2); z < (RADIUS / 2); z++) {
                        ItemStack foundStack = new ItemStack(
                            par2World.getBlockId(playerX + x, y, playerZ + z),
                            1,
                            par2World.getBlockMetadata(playerX + x, y, playerZ + z));
            ...
            this.printMessage("", par3EntityPlayer);
            this.printMessage("Found materials in " + RADIUS + "x" + RADIUS + " cubouid below you", par3EntityPlayer);
            this.printMessage("", par3EntityPlayer);
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ADV_SCANNER_TEXTURE);
    }

...
public class ItemElectricSickle extends ItemTool implements IElectricItem, IHasRecipe {
    ...
    public ItemElectricSickle(int id) {
        super(id, 0, EnumToolMaterial.IRON, new Block[] { Block.leaves, Block.tallGrass });
    ...
    public boolean onBlockDestroyed(ItemStack par1ItemStack, World par2World, int blockId, int x, int y, int z,
        EntityLivingBase par7EntityLivingBase) {
        if (!par2World.isRemote) {
            if (ElectricItem.manager.canUse(par1ItemStack, ENERGY_PER_BLOCK)) {
                ElectricItem.manager.use(par1ItemStack, ENERGY_PER_BLOCK, par7EntityLivingBase);
                        ...
                        if (blockBroken instanceof IPlantable) {
                            ...
                            for (BlockLocation curr : blocksToBreak) {
                                ...
                                Block currBlock = Block.blocksList[par2World
                                    .getBlockId(curr.getX(), curr.getY(), curr.getZ())];
                                if (currBlock != null) {
                                    // System.out.println(currBlock.getClass()
                                    // .getName());
                                    if (currBlock instanceof BlockTallGrass || currBlock instanceof IPlantable) {
                                        // System.out.println("breakingblock");
                                        if (currBlock.removeBlockByPlayer(
                                            ...
                                            curr.getZ())) {
                                            if (ElectricItem.manager.canUse(par1ItemStack, ENERGY_PER_BLOCK))
                                                ElectricItem.manager.use(par1ItemStack, ENERGY_PER_BLOCK, player);
                                            else {
                                                break;
                                            }
                                            currBlock.dropBlockAsItem(
                                                ...
                                                curr.getY(),
                                                curr.getZ(),
                                                par2World.getBlockMetadata(curr.getX(), curr.getY(), curr.getZ()),
                                                2);
                                        }
                                    }
                                }
                        ...
                        if (blockBroken.isLeaves(par2World, x, y, z)) {
                            ...
                            for (BlockLocation curr : blocksToBreak) {
                                Block currBlock = Block.blocksList[par2World
                                    .getBlockId(curr.getX(), curr.getY(), curr.getZ())];
                                if (currBlock != null) {
                                    if (currBlock.isLeaves(par2World, curr.getX(), curr.getY(), curr.getZ())) {
                                        if (currBlock.removeBlockByPlayer(
                                            par2World,
                                            (EntityPlayer) par7EntityLivingBase,
                                            curr.getX(),
    ...
    public void registerIcons(IconRegister par1IconRegister) {
        this.itemIcon = par1IconRegister.registerIcon(ClientProxy.ELECTRIC_SICKLE_TEXTURE);
    }
    ...
    public void addCraftingRecipe() {
        ...
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemElectricSickle),
            "  i",
            "eii",
            "r  ",
            Character.valueOf('i'),
            IC2Items.REFINED_IRON,
            Character.valueOf('e'),
            IC2Items.BASIC_CIRCUIT,
            Character.valueOf('r'),
            IC2Items.RE_BATTERY);
        Recipes.advRecipes.addRecipe(
            new ItemStack(LSBlockItemList.itemElectricSickle),

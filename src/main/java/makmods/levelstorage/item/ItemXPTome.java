...
public class ItemXPTome extends Item implements IHasRecipe {
    ...
    public static int XP_PER_BOOKCRAFT = Config.getInt(
        LSConfigCategory.BALANCE,
        "xpPerBookEnchantment",
        840,
        "Determines how much XP is consumed from XP tome when you try to enchant a book with it (default V while holding XP tome in your hand and having books in inventory");
    ...
    public void addCraftingRecipe() {
        // Book
        ItemStack stackDepleted = new ItemStack(LSBlockItemList.itemLevelStorageBook, 1, 0);
        ...
        GameRegistry.addShapelessRecipe(stackDepleted, stackBook, stackGoldBlock, stackEnchTable);
        ...
        if (!par2World.isRemote) {
            ...
            if (par3EntityPlayer.isSneaking()) {
                if ((this.bookMaxStorage - getStoredXP(par1ItemStack)) > par3EntityPlayer.experienceTotal) {
                    increaseStoredXP(par1ItemStack, par3EntityPlayer.experienceTotal);
                ...
                if (getStoredXP(par1ItemStack) > xpPerInteraction) {
                    ...
                    float f = 5 / 30.0F;
                    par2World.playSoundAtEntity(par3EntityPlayer, "random.levelup", f * 0.75F, 1.0F);
                } else {
                    if (getStoredXP(par1ItemStack) != 0) {
                        par3EntityPlayer.addExperience(getStoredXP(par1ItemStack));
                        par1ItemStack.stackTagCompound.setInteger(STORED_XP_NBT, 0);
                        float f = 5 / 30.0F;
                        par2World.playSoundAtEntity(par3EntityPlayer, "random.levelup", f * 0.75F, 1.0F);
                    }
                }
    ...
    public ItemStack generateRandomBook(Random rand) {
        ...
        for (Enchantment ench : Enchantment.enchantmentsBookList) if (ench != null) availableIDs.add(ench);
    ...
    public void attemptCreateEnchBook(ItemStack xpTome, World par2World, EntityPlayer player) {
        if (!(getStoredXP(xpTome) >= XP_PER_BOOKCRAFT)) return;
    ...
    public void onUpdate(ItemStack par1ItemStack, World par2World, Entity par3Entity, int par4, boolean par5) {
        if (!par2World.isRemote) {
            ...
            if (par3Entity instanceof EntityPlayerMP) {
                ...
                if (player.inventory.getCurrentItem() != par1ItemStack) return;
                if (LSKeyboard.getInstance()
                    .isKeyDown((EntityPlayer) par3Entity, LSKeyboard.RANGE_KEY_NAME)) {
                    if (Cooldownable.use(par1ItemStack, COOLDOWN * 4)) {
                        attemptCreateEnchBook(par1ItemStack, par2World, (EntityPlayer) par3Entity);
                    }
                }
            }
        }
    ...
    public void addInformation(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, List par3List, boolean par4) {
        ...
        par3List.add(
            StatCollector.translateToLocal("tooltip.xptome.stored") + " "
    ...
    public static int calculateDurability(ItemStack stack) {
        ...
        int durability = stack.getMaxDamage() - (int) (stack.getMaxDamage() * percent);
        if (durability == 0) {
            durability = 1;
        }
        return durability;
    ...
    public void getSubItems(int par1, CreativeTabs par2CreativeTabs, List par3List) {
        ...
        stackFull.stackTagCompound.setInteger(STORED_XP_NBT, this.bookMaxStorage - 1);
        ItemStack stackDepleted = new ItemStack(par1, 1, this.getMaxDamage() - 1);
        stackDepleted.stackTagCompound = new NBTTagCompound();

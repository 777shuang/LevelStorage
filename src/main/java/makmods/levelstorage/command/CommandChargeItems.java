...
public class CommandChargeItems extends CommandBase {
    ...
    public List addTabCompletionOptions(ICommandSender par1ICommandSender, String[] args) {
        if (args.length == 1) return getListOfStringsMatchingLastWord(args, getPlayers());
        return null;
    }
    ...
    public int chargeItems(EntityPlayerMP player) {
        ...
        for (ItemStack stack : inv.armorInventory) if (stack != null) if (stack.getItem() instanceof IElectricItem)
            charged += ElectricItem.manager.charge(stack, Integer.MAX_VALUE, 10, true, false);
        for (ItemStack stack : inv.mainInventory) if (stack != null) if (stack.getItem() instanceof IElectricItem)
            charged += ElectricItem.manager.charge(stack, Integer.MAX_VALUE, 10, true, false);
        return charged;
    }
    ...
    public void processCommand(ICommandSender icommandsender, String[] astring) {
        if (astring.length == 1) {
            ...
            EntityPlayerMP player = MinecraftServer.getServerConfigurationManager(MinecraftServer.getServer())
        ...
        icommandsender.sendChatToPlayer(
            ChatMessageComponent
                .createFromText("\247dCharged all items inside your inventory. (used " + charged + " EU)"));
    }
